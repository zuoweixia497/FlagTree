...
sunrisemmaDotOperandToLinearLayout(DotOperandEncodingAttr dotEncAttr,
  ...
  bool kContig = (dotOpIdx == 0 ? sunriseMmaAttr.getIsACol() == false
                                : sunriseMmaAttr.getIsBCol() == true);
  auto order = getOrderForDotOperand(dotOpIdx, rank, kContig);
  unsigned elemBitWidth = sunriseMmaAttr.getInputElemBitWidth();
  auto tileLayout = LinearLayout::empty();
  switch (elemBitWidth) {
  case 32:
    if ((dotOpIdx == 0 && sunriseMmaAttr.getIsACol() == false) ||
        (dotOpIdx == 1 && sunriseMmaAttr.getIsBCol() == true)) {
      tileLayout =
  ...
  default:
    llvm::report_fatal_error(
        (std::string("linearlayout not implemented! elemBitWidth:") +
         std::to_string(elemBitWidth))
            .c_str());
    break;
  }
