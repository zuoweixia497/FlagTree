...
class dtype(base_type):
    ...
    def to_ir(self, builder: ir.builder) -> ir.type:
        if self.name.startswith("fp8"):
            supported_fp8_dtypes = getattr(
                getattr(builder, "options", None),
                "supported_fp8_storage_dtypes",
                getattr(getattr(builder, "options", None), "supported_fp8_dtypes", ()),
            )
            if supported_fp8_dtypes and self.name not in supported_fp8_dtypes:
                raise ValueError(f'type {self} not supported in this architecture. '
                                 f'The supported fp8 dtypes are {supported_fp8_dtypes}')
...
def _experimental_descriptor_load(desc_pointer, offsets, shape, dtype, _semantic=None):
    ...
    New code should prefer `load_tensor_descriptor`. We keep this symbol so
    migrated tests can exercise the same backend descriptor path without
    monkeypatching triton.language internals in conftest.
    """
    # Legacy signature includes shape; 3.5 descriptors already encode block
    # shape in the descriptor type, so shape is ignored.
    _ = shape
    dtype = _unwrap_if_constexpr(dtype)
