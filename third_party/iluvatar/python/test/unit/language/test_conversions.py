...
from triton._internal_testing import is_cuda, is_hip, is_hip_cdna2, is_hip_cdna3, is_hip_cdna4, is_hip_gfx12, is_corex
...
def test_typeconvert_upcast(src_dtype, dst_dtype, device):
    ...
    if is_cuda() and not is_corex():
        ...
        if src_dtype in ('float8e4b8', 'float8e5b16'):
            pytest.skip(f"upcasting {src_dtype} is not enabled on ivcore11 yet")
...
def test_typeconvert_downcast(src_dtype, dst_dtype, rounding, max_repr, device):
    ...
    if is_corex():
        if dst_dtype in ('float8e4b8', 'float8e5b16'):
            pytest.skip(f"{dst_dtype} downcast is not enabled on ivcore11 yet")
        if rounding == 'rtz' and dst_dtype in ('float8e5', 'float8e4nv'):
            pytest.skip("RTZ downcast to OCP float8 is not enabled on ivcore11 yet")
...
def test_typeconvert_downcast_clamping(src_dtype, dst_dtype, mode, device, rounding="rtne"):
    ...
    if is_corex() and dst_dtype in ('float8e4b8', 'float8e5b16'):
        pytest.skip(f"{dst_dtype} downcast is not enabled on ivcore11 yet")
